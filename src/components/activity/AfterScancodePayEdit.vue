        ...
        <Row>
          ...
          <Col span="6">
            <FormItem label="开始时间" prop="starttime">
              <DatePicker type="date" placeholder="选择时间" :value="addData.starttime" @on-change="e => addData.starttime = e" style="width:100%"></DatePicker>
            </FormItem>
          </Col>
          <Col span="6">
            <FormItem label="结束时间" prop="endtime">
              <DatePicker type="date" placeholder="选择时间" :value="addData.endtime" @on-change="e => addData.endtime = e" style="width:100%"></DatePicker>
            </FormItem>
          </Col>
          <Col span="6">
...
export default {
  ...
  methods: {
    getActData () {
      let d = {
        codepayid: this.$route.params.id
      }
      serverApi('/paymentactive/index', d,
        response => {
          this.isloading = false
          console.log(response)
          if (response.data.code === 0) {
            this.addData = response.data.data
            this.cityid = response.data.data.codepaycity.split(',').map(e => Number(e))
            this.rulesArr = response.data.data.content
