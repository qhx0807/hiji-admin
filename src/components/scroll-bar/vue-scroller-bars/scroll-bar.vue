...
export default {
  ...
  computed: {
    ...
    contentStyles () {
      return {
        transform: `translate3d(-${this.scrollOffsetX}px, -${
          this.scrollOffsetY
        }px, 0px)`,
    ...
    scrollBarYStyles () {
      ...
      return {
        transform: `translate3d(0px, ${this.scrollOffsetY *
          (height / this.wraperSize.height)}px, 0px)`,
        height: `${height}px`
      }
    },
    scrollYClasses () {
    ...
    scrollBarXStyles () {
      ...
      return {
        transform: `translate3d(${this.scrollOffsetX *
          (width / this.wraperSize.width)}px, 0px, 0px)`,
        width: `${width}px`
      }
    },
    scrollXClasses () {
  ...
  methods: {
    ...
    handleMousemoveY (e) {
      ...
      this.scrollOffsetY =
        this.initOffsetY +
        offset /
          ((this.wraperSize.height - 2 - this.scrollBarYHeight) /
            (this.contentSize.height - this.wraperSize.height))
      if (this.scrollOffsetY >= this.gapY) {
        this.scrollOffsetY = Math.min(this.gapY, this.scrollOffsetY)
      } else if (this.scrollOffsetY <= 0) {
        this.scrollOffsetY = Math.max(this.scrollOffsetY, 0)
    ...
    handleMousemoveX (e) {
      ...
      this.scrollOffsetX =
        this.initOffsetX +
        offset /
          ((this.wraperSize.width - 2 - this.scrollBarXWidth) /
            (this.contentSize.width - this.wraperSize.width))
      if (this.scrollOffsetX >= this.gapX) {
        this.scrollOffsetX = Math.min(this.gapX, this.scrollOffsetX)
      } else if (this.scrollOffsetX <= 0) {
        this.scrollOffsetX = Math.max(this.scrollOffsetX, 0)
